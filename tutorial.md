####EcmaScript6 introduces a native Promises spec which "promises" to make asynchronous chaining much simpler.
...
No doubt you have written plently of JavaScript synchronous code, but I want to point out this most basic form of operation in case you have never considered synchronous code distinct from asynchronous code.
...
Side Note**Creating global variables is usually discouraged, a better way of capturing a value in the global scope is to create an object in the global space and storing it in a key value pair. Or one can avoid the global context by placing all code inside a function body. That way the scope of variables on the top level is shielded from the global context and only the function name if given one will reside in the global name space.

To chain asynchronous functions together they have to be nested so that they are forced to operate in sequence. Passing variables to a nested function works because all functions declared inside an outer function have access to it's variables. Checkout [scope](http://toddmotto.com/everything-you-wanted-to-know-about-javascript-scope/) for more information on how variables are accessed in JavaScript functions.
...
Regardless of whether a function is synchronous or asynchronous nesting forces the operation of subsequent functions to occur sequentially, where as chaining functions will allow asynchronous code to occur in the background while the functions are passing values synchronously. This means that the values getting passed are not the final computed values that the asynchronous functions will eventually give.
...
Passing variables and sequential operation are two difficult aspects of asynchronous code that Promises were designed to make simpler.
...
<div class="side-note">If you get an error stating that Promise is not defined your browser may not support Promises( currently no version of IE does). Consider updating your browser or include the polyfill recommended by [Forbes Lindesay](https://www.promisejs.org).</div>
